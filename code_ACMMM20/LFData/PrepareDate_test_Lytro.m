...
listname = sprintf('%s_test.txt',dataset);
f = fopen(listname);
if( f == -1 )
...
for k = 1:size(list,1)
    ...
    lf_rgb = zeros(H,W,3,allah,allaw,'uint8');
    lf_ycbcr = zeros(H,W,3,allah,allaw,'uint8');

    % HCI dataset
    for v = 1 : allah
        for u = 1 : allaw
            sub = eslf(v:allah:end,u:allah:end,:); 
            sub = sub(startcrop_h:H+startcrop_h-1,startcrop_w:W+startcrop_w-1,:);
            lf_rgb(:,:,:,v,u) = sub;
            lf_ycbcr(:,:,:,v,u) = rgb2ycbcr(sub);                
        end
    end        

    lf_rgb = lf_rgb(:,:,:,4:11,4:11);
    lf_ycbcr = lf_ycbcr(:,:,:,4:11,4:11);
