...
def CropPatches_rgb(image,len,crop):
    #left [1,an2,3,h,lw]
    #middles[n,an2,3,h,mw]
    #right [1,an2,3,h,rw]
    an,_,h,w = image.shape[1:5]
    left = image[:,:,:,:,0:len+crop]
    num = floor((w-len-crop)/len)
    middles = torch.Tensor(num,an,3,h,len+crop*2).to(image.device)
    for i in range(num):
        middles[i] = image[0,:,:,:,(i+1)*len-crop:(i+2)*len+crop]
    right = image[:,:,:,:,-(len+crop):]
    return left,middles,right

def MergePatches_rgb(left,middles,right,h,w,len,crop):

    n,a = left.shape[0:2]
    out = torch.Tensor(n,a,3,h,w).to(left.device)
    out[:,:,:,:,:len] = left[:,:,:,:,-crop]
    for i in range(middles.shape[0]):
        out[:,:,:,:,len*(i+1):len*(i+2)] = middles[i:i+1,:,:,:,crop:-crop]
    out[:,:,:,:,-len:]=right[:,:,:,:,crop:]
