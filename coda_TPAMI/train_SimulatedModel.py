...
from Dataset_Simulated import DatasetFromHdf5
...
def train(epoch):
    ...
    for k in range(50):  
        for i, batch in enumerate(train_loader, 1):
            # print(i)
            label, lr, hr = batch[0].to(device), batch[1].to(device), batch[2].to(device)
            with torch.autograd.set_detect_anomaly(True):
                # forward pass
                sr_copy, mask_copy, disp, sr_warp, mask_warp, sr_fusion = model(lr, hr)
